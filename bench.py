...
gateStates = ["NAND", "OR", "XOR", "AND", "NOR"]
...
def iterateSolveandNormalize(stageModule, targetfile, design):
	...
	if not len(remFO) and not len(remFI):
		if not check_reconvergent(stageModule, design):
			return False
		print("design sucess!")
...
def designFramework(stageModule, targetfile, design):
	...
	for i in range(0,design.mainIterate):	
		status = iterateSolveandNormalize(stageModule, targetfile, design)
		if status == True:
			break
